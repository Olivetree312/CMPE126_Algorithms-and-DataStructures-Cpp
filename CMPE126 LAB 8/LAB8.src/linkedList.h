...
public:
	...
	//not sure how to do this without helper function
	bool seqSearchRec(T elem){
		return seqSearchRecHelper(elem, head);
	}
	bool seqSearchRecHelper(T elem, Node<T>* n){
		if(n==nullptr){
			return false;
		}
		if(n->data==elem){
		...
		return seqSearchRecHelper(elem, n->next);
	}
