...
//***BASE CLASS CONSTRUCTORS MUST BE INVOKED W INITIALIZATION LIST
template <class T> sortedLinkedList<T>::sortedLinkedList
	(Node<T>* head, int maxSize): linkedList<T>(head, maxSize){
	sortList();
}
template <class T> sortedLinkedList<T>::sortedLinkedList(linkedList<T>& list): linkedList<T>(list){
	sortList();
}
//place Nodes in vector, then sorts
//switches values, doesn't affect Node connections
//***USE this-> when directly accessing members of base class
template <class T> void sortedLinkedList<T>::sortList(){
	vector<T> arr;
	//place data in vector
...
template class sortedLinkedList<double>;
template class sortedLinkedList<bool>;
template class sortedLinkedList<float>;
